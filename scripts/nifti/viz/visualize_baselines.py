...
`scripts/nifti/metrics/evaluate_baseline.py`, dynamically detects the
top-performing model run, and produces publication-quality charts (metric
boxplots, comparative ROC curves, isolated ROC curve for the best run, and
...
def get_optimal_run_id(results_json_path: str) -> str:
    ...
    Args:
        results_json_path: Filepath to the serialized baseline results JSON.
    ...
    best_run = max(
        runs,
        key=lambda item: (item.get("accuracy", 0.0), item.get("auc", 0.0)),
    )
...
def main() -> None:
    ...
    results_json_path = "./data/results/metrics/nifti/baseline_results.json"
    plots_output_dir = "./data/results/plots/nifti"

    if not os.path.exists(results_json_path):
        logger.error(f"Results file not found at: {results_json_path}")
        logger.error("Please execute scripts/nifti/metrics/evaluate_baseline.py first.")
        sys.exit(1)
