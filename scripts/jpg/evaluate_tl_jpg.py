...
def main() -> None:
    logger = setup_logger("tl_evaluator_jpg")
    device = get_device()
    models_dir = "./data/results/trained_tl_models/jpg"
    ...
    if not model_files:
        logger.error("No TL .pt files found.")
        sys.exit(1)
    ...
    logger.info(f"Success! TL evaluation complete. Results saved to {output_json}")
    logger.info(
