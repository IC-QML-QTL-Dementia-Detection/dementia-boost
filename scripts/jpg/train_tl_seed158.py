...
def main() -> None:
    logger = setup_logger("classical_tl_train_jpg")
    device = get_device()

    experiment_seed = 158
    run_id = f"seed_{experiment_seed}"
    baseline_weights_path = f"./data/results/trained_models/jpg/baseline_{run_id}.pt"
    ...
    if not os.path.exists(baseline_weights_path):
        logger.error(f"Baseline weights not found at: {baseline_weights_path}")
        sys.exit(1)
    ...
    run_id = f"tl_seed_{experiment_seed}"
    trainer.train(epochs=epochs, run_id=run_id)
